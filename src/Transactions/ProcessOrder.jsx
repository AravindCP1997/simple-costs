...
import { ProcessOrder, Plant, CompanyCollection, Material } from "../classes";

import { defaultProcessOrder } from "../defaults";

export function CreateProcessOrder({
  initial = defaultProcessOrder,
  meth = "Create",
}) {
  const {
    data: promptData,
  ...
  const promptCollection = new ProcessOrder(
    promptData.code,
    promptData.company,
  );
  ...
  const { Company, Code, Date, PlantCode, Description, Products, Status } =
    processed;
  const collection = new ProcessOrder(Code, Company);
      ...
      <WindowTitle
        ...
        menu={[
          ...
          <HidingPrompt
            ...
            title={"Open"}
            onClose={[() => resetPrompt()]}
            onSubmitFail={[() => showAlert("Process order does not exist.")]}
            onSubmitSuccess={[
