...
import { VendorGroup, GeneralLedger } from "../classes";
...
import { defaultVendorGroup } from "../defaults";

export function ManageVendorGroup() {
  const [company, setcompany] = useState("");
  const [code, setcode] = useState("");
  const { openWindow, openConfirm, showAlert } = useInterface();
  const collection = new VendorGroup(code, company);
    ...
    <>
      <WindowTitle
        ...
        menu={[
          <Button
            name="New"
            functionsArray={[() => openWindow(<CreateVendorGroup />)]}
          />,
          <ConditionalButton
            name={"View"}
            result={collection.exists()}
            whileFalse={[() => showAlert("Vendor Group does not exist.")]}
            whileTrue={[
              () =>
                openWindow(
                  <CreateVendorGroup
...
export function CreateVendorGroup({
  initial = defaultVendorGroup,
  method = "Create",
}) {
  ...
  useEffect(() => {
    ...
    if (method === "Create") {
      ...
      addError(
        collection.exists(),
        "Code",
        `Vendor Group ${Code} already exist in Company ${Company}.`,
      );
      Gls.forEach((gl, g) => {
        addError(
          collection.company.exists() &&
