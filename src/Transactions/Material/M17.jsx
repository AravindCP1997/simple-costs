...
import { Company, StockTransportOrder, STOIssue } from "../../classes";
import { isPositive, perform, transformObject } from "../../functions";

export function MaterialReceiptSTO() {
  const defaults = {
    CompanyCode: "",
    ValueDate: "",
    MatYear: "",
      ...
      matData.Movements.push(
        ...[
          transformObject(
            ...
            [],
            { LocationCode: Location, MovementType: "17", Block: "Free" },
          ),
        ],
      );
    });
  ...
  useEffect(() => {
    ...
    Items.forEach((item, i) => {
      ...
      addError(
        ...
        `Items/${i + 1}`,
        "Quantity exceeds Quantity in Transit.",
      );
    });
  }, [data]);
  ...
  return (
    <>
      <WindowTitle
        title={"Material Receipt Against STO"}
        menu={[
          <ConditionalButton
            name={"Post"}
            result={!errorsExist}
                      ...
                      changeData(
                        ...
                        issue.issues().map((item, i) =>
                          transformObject(
                            ...
                            {
                              Remarks: "",
                              Quantity: issue.inTransitQuantity(item.No),
                              Value: 0,
                            },
                          ),
                        ),
