...
export function MaterialIssueUnit() {
  const defaults = {
    CompanyCode: "",
    ValueDate: "",
    AccYear: "",
  ...
  const collection = company.collection(UnitType);
  ...
  useEffect(() => {
    ...
    addError(
      !collection.exists({ Code: Unit }),
      UnitType,
      `${UnitType} ${Unit} does not exist.`,
    );
    addError(
  ...
  return (
    <>
      <WindowTitle
        title={"Material Issue Against Units"}
        menu={[
          <ConditionalButton
            name={"Post"}
            result={!errorsExist}
      ...
      <WindowContent>
        <DisplayArea>
          ...
          <Row overflow="visible">
            ...
            <Option
              value={UnitType}
              process={(value) => changeData("", "UnitType", value)}
              options={["CostCenter", "RevenueCenter", "Location", "Plant"]}
            />
          </Row>
          <Row overflow="visible">
            <Label label={"Unit"} />
