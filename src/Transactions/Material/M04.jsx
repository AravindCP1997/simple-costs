...
export function ConsignmentInwardsLoss() {
  const defaults = {
    CompanyCode: "",
    ValueDate: "",
    MatYear: "",
      ...
      matData.Movements.push(
        ...[
          transformObject(
            ...
            [],
            { LocationCode: Location, MovementType: "04", Block: "Free" },
          ),
        ],
      );
  ...
  useEffect(() => {
    ...
    Items.forEach((item, i) => {
      ...
      const CI = new ConsignmentInwards(CompanyCode, RefYear, RefDocNo);
      addError(
        Quantity === "" || Quantity <= 0,
        `Items/${i + 1}`,
      ...
      addError(
        CI.inTransitQuantity(RefItem) < Quantity,
        `Items/${i + 1}`,
  ...
  return (
    <>
      <WindowTitle
        title={"Consignment Inwards Loss"}
        menu={[
          <ConditionalButton
            name={"Post"}
            result={!errorsExist}
