...
export class Employee extends CompanyCollection {
  ...
  taxableWageSheet(year, month, calculateFrom = monthBegin(year, month)) {
    ...
    data.forEach((record) => {
      ...
      const type = wagetype.getData().Type;
      const RevisedAmount =
        type === "Emolument" ? Math.round(Amount) : -Math.round(Amount);
      const Taxable = taxability ? RevisedAmount : 0;
  ...
  standarddeduction(year, taxableWage) {
    const taxcode = this.taxCode(year);
    if (taxcode.exists() && taxcode.yearExists(year)) {
      return taxcode.standardDeduction(year, taxableWage);
    }
  }
  tax(year, income) {
    const taxcode = this.taxCode(year);
  ...
  postedTDS(year) {
    ...
    const filtered = data.filter((record) =>
      this.company.year(monthBegin(record.Year, record.Month) === year),
    );
    return SumField(filtered, "Amount");
  }
  ...
  postedTaxableWage(year) {
    const data = filterByMultipleSelection(PostedRemunerationTable(), [
      filter("Company", "StringCaseInsensitive", [this.companycode]),
      filter("Employee", "Number", [this.code]),
      filter("WT", "StringCaseInsensitive", ["TWAGE"]),
    ]);
    const filtered = data.filter((record) =>
      this.company.year(monthBegin(record.Year, record.Month) === year),
    );
  ...
  postedTaxableWageByOrgUnit(year, unit) {
    const data = filterByMultipleSelection(PostedRemunerationTable(), [
      filter("Company", "StringCaseInsensitive", [this.companycode]),
      filter("Employee", "Number", [this.code]),
      filter("WT", "StringCaseInsensitive", ["TWAGE"]),
...
export const EntryTypes = {
  ...
  opposite: function (code) {
    return this.getField(code, "O");
  },
  getDescription: function (ET, Company, Account) {
    const collectionname = this.getField(ET, "A");
    const data = new CompanyCollection(Company, collectionname).getData({
      Code: Account,
    });
    if (["Vendor", "Customer"].includes(collectionname)) {
      return data.Name;
    }
...
export class RemunerationResult extends Collection {
  ...
  slip() {
    ...
    const WTs = allWTs.filter(
      (item) => !["WHT", "NPAY", "DUE", "RDUE", "PMT", "TWAGE"].includes(item),
    );
...
export class RemunerationCalc {
  ...
  restOfYearTaxableWage() {
    ...
    const postedTaxableWage = this.employee.postedTaxableWage(this.FY());
    let forecastTaxableWage = 0;
    months.forEach((record) => {
      const [Year, Month] = record;
  ...
  taxyear() {
    return this.company.year(monthBegin(this.year, this.month));
  }
  standardDeduction() {
    return this.employee.standarddeduction(
      this.taxyear(),
      this.yearlyTaxableWage(),
    );
  }
  additions() {
    return this.employee.additions(this.taxyear());
  }
  deductions() {
    return this.employee.deductions(this.taxyear());
  }
  totalIncome() {
    return (
      this.yearlyTaxableWage() +
  ...
  tax() {
    return this.employee.tax(this.taxyear(), this.totalIncome());
  }
  taxRate() {
    return this.tax() / this.yearlyTaxableWage();
  }
  withholdingTax() {
    const postedWithHoldingTax = this.employee.postedTDS(this.FY());
  ...
  restOfYearTaxableWageOfOrgUnit(unit) {
    ...
    const postedTaxableWage = this.employee.postedTaxableWageByOrgUnit(
      this.FY(),
      unit,
    );
    let forecastTaxableWage = 0;
      ...
      result.push(
        ...[
          ...
          },
          {
            ...
            From,
            To,
            Amount: this.taxableWageOfOrgUnit(unit),
          },
        ],
      );
    });
...
export class RemunerationExpensePosting extends Collection {
  constructor(CompanyCode, Year, Month) {
    super("RemunerationExpensePosting");
    this.companycode = CompanyCode;
    this.year = Year;
    this.month = Month;
    this.company = new Company(this.companycode);
  ...
  postedData() {
    const data = filterByMultipleSelection(PostedRemunerationTable(), [
      ...
      filter("Type", "StringCaseSensitive", ["Regular"]),
    ]);
    return data;
  }
  dataExists() {
    return this.postedData().length > 0;
  }
  transformedData() {
  ...
  wagesEntry() {
    const data = this.transformedData().filter(
      (item) =>
        !["WHT", "NPAY", "TWAGE", "PMT", "DUE", "RDUE"].includes(item.WT),
    );
    const WTs = ListUniqueItems(data, "WT");
    ...
    WTs.forEach((WT) => {
      ...
      PCs.forEach((PC) => {
        ...
        result.push({
          ...
          Amount,
          ET: Amount >= 0 ? "G1" : "G2",
          Text: `Remuneration ${monthEnd(this.year, this.month)}, Wage Type ${WT}`,
        });
      });
    });
    return result;
          ...
          ...[
            {
              ...
              PC,
              ET: payable >= 0 ? "G2" : "G1",
              Text: `Remuneration ${monthEnd(this.year, this.month)}, Net Pay`,
            },
            {
              Account: whtGL,
              Amount: wht,
              PC,
              ET: wht >= 0 ? "G2" : "G1",
              Text: `Remuneration ${monthEnd(this.year, this.month)}, Withholding Tax`,
            },
          ],
        );
      });
  ...
  entry() {
    return {
      ...
      DocumentDate: monthEnd(this.year, this.month),
      DocumentType: "SE",
  ...
  processCostingDocument() {
    return new ProcessCostingDocument(this.costEntry(), this.companycode);
  }
  costItems() {
    const data = this.transformedData().filter(
      (item) =>
      ...
      wages.forEach((wage) => {
        ...
        if (CostElements.includes(WT)) {
          ...
          result.push({
            ...
            ObjectType: "Employee",
            Object: employee,
            Text: `Remuneration Payment ${monthEnd(this.year, this.month)}, Wage Type ${WT}`,
          });
        }
      });
    });
  ...
  post() {
    const { DocumentNo: AccountingDocument } =
  ...
  accountingDocument() {
    const { AccountingDocument: AccDocNo, CostingDocument: CostDocNo } =
      this.getData();
    return new AccountingDocument(this.companycode, AccDocNo, this.FY());
  }
  costingDocument() {
    const { AccountingDocument: AccDocNo, CostingDocument: CostDocNo } =
      this.getData();
    return new CostingDocument(this.companycode, CostDocNo, this.FY());
  }
  reverse() {
    const AccountingDocumentNo = this.accountingDocument().reverse(
      monthEnd(this.year, this.month),
    );
    const CostingDocumentNo = this.costingDocument().reverse(
      monthEnd(this.year, this.month),
    );
    this.update({ AccountingDocument: "", CostingDocument: "", Posted: false });
    return `Reversed, 'Accounting Document No: ${AccountingDocumentNo}, Costing Document No: ${CostingDocumentNo}`;
  }
}
...
export class RemunerationOffcycleCalc {
  ...
  wht() {
    return Math.round(this.taxableWage() * this.taxrate());
  }
  netwages() {
    return SumField(this.wagesheet(), "Amount");
  }
...
export class RemunerationOffcycleExpensePosting extends Collection {
  constructor(
    CompanyCode,
    OffCycleDate,
    Year = year(OffCycleDate),
    Month = month(OffCycleDate),
  ) {
    super("RemunerationExpensePosting");
    this.companycode = CompanyCode;
    this.year = Year;
    this.month = Month;
    this.date = OffCycleDate;
    this.company = new Company(this.companycode);
    this.criteria = {
      Company: this.companycode,
      Date: this.date,
  ...
  postedData() {
    const data = filterByMultipleSelection(PostedRemunerationTable(), [
      filter("Company", "StringCaseSensitive", [this.companycode]),
      filter("Date", "StringCaseSensitive", [this.date]),
      filter("Type", "StringCaseSensitive", ["OffCycle"]),
    ]);
    return data;
  }
  dataExists() {
    ...
    WTs.forEach((WT) => {
      ...
      PCs.forEach((PC) => {
        ...
        result.push({
          ...
          Amount,
          ET: Amount >= 0 ? "G1" : "G2",
          Text: `Remuneration ${this.date}, Wage Type ${WT}`,
        });
      });
    });
    return result;
          ...
          ...[
            {
              ...
              PC,
              ET: payable >= 0 ? "G2" : "G1",
              Text: `Remuneration ${this.date}, Net Pay`,
            },
            {
              Account: whtGL,
              Amount: wht,
              PC,
              ET: wht >= 0 ? "G2" : "G1",
              Text: `Remuneration ${this.date}, Withholding Tax`,
            },
          ],
        );
      });
  ...
  entry() {
    return {
      ...
      DocumentType: "SE",
      CreationInfo: `${this.date}`,
      Currency: this.company.getData().Currency,
      Text: `Remuneration Expense Posting ${this.date}`,
      Entries: [...this.wagesEntry(), ...this.payablesEntry()],
    };
  }
  processAccountingDocument() {
      ...
      wages.forEach((wage) => {
        ...
        if (CostElements.includes(WT)) {
          ...
          result.push({
            ...
            ObjectType: "Employee",
            Object: employee,
            Text: `Remuneration Payment ${this.date}, Wage Type ${WT}`,
          });
        }
      });
    });
  ...
  reverse() {
    const AccountingDocumentNo = this.accountingDocument().reverse(this.date);
    const CostingDocumentNo = this.costingDocument().reverse(this.date);
    this.update({ AccountingDocument: "", CostingDocument: "", Posted: false });
    return `Reversed, 'Accounting Document No: ${AccountingDocumentNo}, Costing Document No: ${CostingDocumentNo}`;
  }
}
...
export class CostingDocument extends CompanyCollection {
  constructor(company, documentNo, year, name = "CostingDocument") {
    super(company, name);
    this.documentNo = Number(documentNo);
    this.year = year;
    this.criteria = {
...
export class ProcessCostingDocument {
  constructor(data = {}, company = data.Company) {
    this.data = data;
    this.company = new Company(company);
    this.companycode = company;
  ...
  processedEntry(entry) {
    ...
    const ER = this.exchangeRate();
    const { Element, Amount, ObjectType, Object } = data;
  ...
  errors() {
    ...
    const { PostingDate } = this.processed();
    addError(PostingDate === "", "PostingDate", "Posting Date blank.");
    addError(
      this.company.exists() &&
        !this.company.openperiods.costingOpen(PostingDate),
      "PostingDate",
      "Posting Date not open.",
    );
    return list;
  ...
  add() {
    return this.costingDocument().add(this.processed());
  }
  update() {
...
export class RemunerationPayment extends Collection {
  ...
  paymentFile() {
    const data = this.netPayableData().filter((item) =>
      isPositive(item.Amount),
    );
    const Employees = ListUniqueItems(data, "Employee");
    const result = [];
    Employees.forEach((employee) => {
      const emaster = new Employee(employee, this.companycode);
      const { Bank, Account, SWIFT } = this.payableData().find(
        (record) => record.Employee === employee,
      );
      result.push({
        Payee: emaster.getData().Name,
        Amount: SumFieldIfs(
          this.netPayableData(),
          "Amount",
          ["Employee"],
  ...
  post() {
    ...
    this.add({
      ...
      AccountingYear: this.company.year(this.postingdate),
      Reversed: false,
    });
  ...
  reverse(postingDate) {
    const AccountingDocumentNo = this.accountingDocument().reverse(postingDate);
    this.update({ ...this.getData(), ["Reversed"]: true });
    return `Reversed, 'Accounting Document No: ${AccountingDocumentNo}.`;
  }
}
          ...
          ...[
            {
              ...
              Amount,
              PC,
              Text: `Remuneration Payment ${this.date}`,
            },
            {
              ET: Amount >= 0 ? "G2" : "G1",
              Account: this.bankMaster().GL,
